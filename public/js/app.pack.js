...
/***/ function(module, exports, __webpack_require__) {
	...
	buf.push("<div style=\"overflow:auto;\"><img style=\"float:left; margin-right: 10px;\" src=\"{{ item.img }}\"/><h2>{{ item.name }}</h2><div><div>Release date: {{ item.release_date }}</div><div>Url: <a href=\"{{ item.url }}\">{{ item.url }}</a></div></div></div><div id=\"variants\" ng-if=\"item.variants\"><h3>Variants</h3><div v-repeat=\"variant in item.variants\" class=\"variant\"><div>{{ variant.name }}</div><img src=\"{{ variant.img || item.img }}\"/></div></div><div><h3>Prices</h3><table><tr><th colspan=\"2\">Supplier</th><th>Warehouses</th><th>Price</th></tr><tr v-repeat=\"price in item.prices | orderBy 'price_usd'\"><td style=\"width: 100px; text-align: center;\"><img src=\"/app/img/suppliers/{{ price.supplier }}.png\" alt=\"{{ item.suppliers[price.supplier].name }}\" style=\"max-width: 100px; max-height: 30px;\"/></td><td>{{ item.suppliers[price.supplier].name }}</td><td>{{ item.suppliers[price.supplier].warehouses }}</td><td>{{ price | formatPrice }}</td></tr></table><div>Last updated: {{ item.prices_updated }}</div></div><a v-link=\"/\" style=\"margin-top: 100px;\">All products</a>");;return buf.join("");
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	buf.push("<table id=\"product-list\"><tr><th></th><th>Name</th><th>Release Date</th><th>Retail Price</th></tr><tr v-repeat=\"items\"><td class=\"img\"><img src=\"{{ img }}\"/></td><td><a v-link=\"/products/{{id}}\">{{ name }}</a><div><span>{{ items[$index] | getVariants }} variants</span></div></td><td>{{ release_date }}</td><td>{{ items[$index] | getPrice }}</td></tr></table>");;return buf.join("");
	}
